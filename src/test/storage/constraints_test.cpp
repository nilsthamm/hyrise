...
class ConstraintsTest : public BaseTest {
 protected:
  ...
  bool _prepare_commit(std::shared_ptr<TransactionContext> context) {
    return context->_prepare_commit();
  }
...
TEST_F(ConstraintsTest, ConcurrentInsertInsertRace) {
  ...
  // With an old implementation of the commit process and the constraint validation, there was a multi-threaded scenario in which both transactions would commit. This is now solved by linearizing the validations. To simulate this multithreaded scenario this test doesn't use the commit() function directly but executes the necessary steps manually.
  ...
  // Normally, the transaction context would now commit all operators in the TransactionContext::commit_async() method and directly try to commit. As the name states, this function is executed asynchronously. In a multi-threaded environment this can lead to the edge case, which is simulated in the following:
  // Even though t2 was first by getting a CommitID, it is not guaranteed t2 will commit all its values before t1. This is simulated by first committing the values of t1 manually
