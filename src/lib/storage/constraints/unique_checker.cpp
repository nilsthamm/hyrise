...
bool check_constraint(std::shared_ptr<const Table> table, const TableConstraintDefinition& constraint, const CommitID& last_commit_id) {
  const auto constraint_checker = std::make_shared<ConcatenatedConstraintChecker>(table, constraint, last_commit_id);
...
bool check_constraints(const std::string &table_name, const CommitID& last_commit_id) {
  std::cout << "Table name: " << table_name << std::endl;
  auto const table = StorageManager::get().get_table(table_name);
  for (const auto& constraint : table->get_unique_constraints()) {
    if (!check_constraint(table, constraint, last_commit_id)) {
      return false;
    }
  }
  return true;
